  ...
  · rename_i x₁ y₁ nonS₁ nonP
    ...
    · rename_i x₂ y₂ nonS₂
      have hxy : ¬(x₁ = x₂ ∧ y₁ = MontgomeryCurveCurve25519.negY x₂ y₂) := by
        simp only [MontgomeryCurveCurve25519, neg_some, WeierstrassCurve.Affine.negY, zero_mul, sub_zero, ne_eq, some.injEq,
    not_and] at PQ
        simp only [WeierstrassCurve.Affine.negY, MontgomeryCurveCurve25519, zero_mul, sub_zero, not_and]
        exact PQ
    ...
    WeierstrassCurve.Affine.negAddY, ite_mul, neg_add_rev]
      ...
      rw[this, neg_some]
      have hxy_neg : ¬(x₁ = x₂ ∧ y₁ = MontgomeryCurveCurve25519.negY x₂ (MontgomeryCurveCurve25519.negY x₂ y₂)) := by
        simp only [WeierstrassCurve.Affine.negY, MontgomeryCurveCurve25519, zero_mul, sub_zero, neg_neg, not_and]
        simp only [MontgomeryCurveCurve25519, ne_eq, some.injEq, not_and] at nonP
        exact nonP
      have h₂ : MontgomeryCurveCurve25519.Nonsingular x₂ (MontgomeryCurveCurve25519.negY x₂ y₂)  := by
        rw[WeierstrassCurve.Affine.nonsingular_neg]
        exact nonS₂
      have := @WeierstrassCurve.Affine.Point.add_some CurveField _ MontgomeryCurveCurve25519 _ x₁ x₂ y₁ (MontgomeryCurveCurve25519.negY x₂ y₂)  hxy_neg nonS₁ h₂
    ...
    true_and] at hxy_neg
        simp only [eq, WeierstrassCurve.Affine.negY, MontgomeryCurveCurve25519, zero_mul, sub_zero, true_and] at hxy
...
theorem uDBL (P : Point) (PZero : P ≠ 0) (nPT : P ≠ T_point) :
  ...
  · have : 2 • P = P + P := by grind
    ...
    · rename_i x y nonS
      ...
      simp only [MontgomeryCurveCurve25519, neg_some, WeierstrassCurve.Affine.negY, zero_mul, sub_zero, some.injEq,
      true_and] at hneg
      simp only [MontgomeryCurveCurve25519, this, WeierstrassCurve.Affine.addX, WeierstrassCurve.Affine.slope, ↓reduceIte,
      WeierstrassCurve.Affine.negY, zero_mul, sub_zero, hneg, sub_neg_eq_add, add_zero, WeierstrassCurve.Affine.negAddY,
      ...
      · have : y+y= 2*y:= by ring_nf
        rw[this]
        field_simp
        simp only [WeierstrassCurve.Affine.Nonsingular, WeierstrassCurve.Affine.equation_iff, ne_eq] at nonS
        have :=nonS.left
...
lemma addX_spec {x₁ x₂ : CurveField} (y₁ y₂ : CurveField) (hx : x₁ ≠ x₂) :
  ...
  simp only [WeierstrassCurve.Affine.addX, MontgomeryCurveCurve25519, Curve25519.A, zero_mul, add_zero, sub_left_inj]
  rw [WeierstrassCurve.Affine.slope, if_neg hx]
