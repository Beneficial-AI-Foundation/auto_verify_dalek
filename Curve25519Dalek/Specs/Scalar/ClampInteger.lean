...
/-- Masking a `U8` with `248 = 0b1111_1000` clears the bottom 3 bits, so the
result is always a multiple of 8. -/
private lemma u8_and_248_mod8 (x : U8) : (x &&& 248#u8).val % 8 = 0 := by
  bvify 8
  bv_decide

/-- Masking a `U8` with `127 = 0b0111_1111` clears the top bit, so the result
is always `< 128`. -/
private lemma u8_and_127_lt_128 (x : U8) : (x &&& 127#u8).val < 128 := by
  bvify 8
  bv_decide

/-- OR-ing a `U8` known to be `< 128` with `64 = 0b0100_0000` sets bit 6
without touching bit 7, so the result stays `< 128`. -/
private lemma u8_or_64_lt_128 (x : U8) (h : x.val < 128) : (x ||| 64#u8).val < 128 := by
  bvify 8 at *
  bv_decide

/-- OR-ing a `U8` with `64 = 0b0100_0000` always sets bit 6, so the result is
`≥ 64`. -/
private lemma u8_or_64_ge_64 (x : U8) : 64 ≤ (x ||| 64#u8).val := by
  bvify 8
  bv_decide
