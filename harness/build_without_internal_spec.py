...
def rewrite_spec_theorem_level(text, closure, keep_names):
...
def main():
    ...
    for rel in sorted(keep):
        if args.theorem_level:
            new, n, cut, kept_names, changed = rewrite_spec_theorem_level(read(rel), closure, theorems[rel])
            ...
            new, n, changed = rewrite_spec(read(rel), closure)
        sorried[rel] = n
        if changed:
    ...
    while closure.stub - done:
        for mod in sorted(closure.stub - done):
            ...
            rel = path_of(mod)
            new, n, changed = rewrite_stub(read(rel), closure)
            stubbed[rel] = n
            if changed:
                import_changes[rel] = changed
            dst = os.path.join(out, rel)
